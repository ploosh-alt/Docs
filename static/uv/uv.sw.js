        ...
        async fetch({ request: i }) {
            ...
            try {
                ...
                return (
                    ...
                    b.intercepted ? b.returnValue : new Response(f.body, { headers: f.headers, status: f.status, statusText: f.statusText })
                );
            } catch (e) {
                          ...
                          return new Response(
                              (function (e, t, r, i, s, o, n) {
                                  if ("The specified host could not be resolved." === i)
                                      return (function (e, t) {
                                          const r = new URL(e),
                                              i = `remoteHostname.textContent = ${JSON.stringify(r.hostname)};bareServer.href = ${JSON.stringify(t)};uvHostname.textContent = ${JSON.stringify(
                                                  location.hostname
                                              )};reload.addEventListener("click", () => location.reload());uvVersion.textContent = ${JSON.stringify("2.0.0")};`;
                                          return `<!DOCTYPE html><html><head><meta charset='utf-8' /><title>Error</title><style>@font-face{font-family:Roboto;font-style:normal;font-weight:400;src:url('/css/Roboto-Regular.ttf') format('woff')}body{background:#000;color:#fff;display:flex;flex-direction:column;justify-content:center;min-height:100vh;margin:0;align-items:center}*{font-family:Roboto}</style></head><body><center><h1>This site can't be reached</h1><hr /><p><b id="remoteHostname"></b>'s server IP address could not be found.</p></center><p>Try:</p><ul><li>Verifying you entered the correct address</li><li>Clearing the site data</li><li>Contacting <b id="uvHostname"></b>'s administrator</li><li>Verifying the <a id='bareServer' title='Bare server'>Bare server</a> isn't censored</li></ul><center><button id="reload">Reload</button><hr /><p><i>Ultraviolet v<span id="uvVersion"></span></i></p><script src="${"data:application/javascript," + encodeURIComponent(i)}"><\/script></center></body></html>`;
                                      })(o, n);
                                  const a =
                                  ...
                                  return (
                                      '<!DOCTYPE html><html><head><meta charset=\'utf-8\' /><title>Error</title><style>@font-face{font-family:Roboto;font-style:normal;font-weight:400;src:url(\'/css/Roboto-Regular.ttf\') format(\'woff\')}body{background:#000;color:#fff;display:flex;flex-direction:column;justify-content:center;min-height:100vh;margin:0;align-items:center}*{font-family:Roboto}</style></head><body><center><h1 id=\'errorTitle\'></h1><hr /><p>Failed to load <b id="fetchedURL"></b></p><p id="errorMessage"></p><table><tbody><tr><td>Code:</td><td id="errorCode"></td></tr>' +
                                      (r ? '<tr><td>ID:</td><td id="errorId"></td></tr>' : "") +
                                      '</tbody></table><textarea id="errorTrace" cols="40" rows="10" readonly></textarea></center><p>Try:</p><ul><li>Checking your internet connection</li><li>Verifying you entered the correct address</li><li>Clearing the site data</li><li>Contacting <b id="uvHostname"></b>\'s administrator</li><li>Verify the <a id=\'bareServer\' title=\'Bare server\'>Bare server</a> isn\'t censored</li></ul><p>If you\'re the administrator of <b id="uvHostname"></b>, try:</p><ul><li>Restarting your Bare server</li><li>Updating Ultraviolet</li><li>Troubleshooting the error on the <a href="https://github.com/titaniumnetwork-dev/Ultraviolet" target="_blank">GitHub repository</a></li></ul><center><button id="reload">Reload</button><hr /><p><i>Ultraviolet v<span id="uvVersion"></span></i></p>' +
                                      `<script src="${"data:application/javascript," + encodeURIComponent(a)}"><\/script></center></body></html>`
                                  );
